...
public struct EonilFSEvents {
    public static func startWatching(paths: [String], for id: ObjectIdentifier, flags: EonilFSEventsCreateFlags? = nil, onQueue queue: DispatchQueue = .main, with handler: @escaping (EonilFSEventsEvent) -> ()) throws {
        ...
        let s = try EonilFSEventStream(
            ...
            sinceWhen: .now,
            latency: 0,
            flags: flags ?? [.noDefer, .fileEvents],
            handler: handler)
        
        s.setDispatchQueue(queue)
        try s.start()
        watchers[id] = s
    }
