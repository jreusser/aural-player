...
class FavoriteAlbumsViewController: NSViewController {
...
extension FavoriteAlbumsViewController: NSTableViewDataSource, NSTableViewDelegate {
    ...
    func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView? {
        ...
        guard let columnId = tableColumn?.identifier,
              ...
              let album = favoritesDelegate.favoriteAlbum(atChronologicalIndex: row)?.groupName else {return nil}
        
        let image = (libraryDelegate.findGroup(named: album, ofType: .album) as? AlbumGroup)?.art ?? .imgAlbumGroup
...
extension FavoriteAlbumsViewController: FontSchemeObserver {
...
extension FavoriteAlbumsViewController: ColorSchemeObserver {
